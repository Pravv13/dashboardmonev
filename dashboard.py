    ...
    if df_hasil.empty:
        ...
        with tab2:
            ...
            if 'Tanggal Laporan' in df_hasil.columns and not df_hasil['Tanggal Laporan'].isna().all():
                df_tren = df_hasil.groupby(df_hasil['Tanggal Laporan'].dt.date).size().reset_index(name='Jumlah Anomali')
                fig_line = px.line(df_tren, x='Tanggal Laporan', y='Jumlah Anomali', markers=True, color_discrete_sequence=['#E91E63'])
            ...
            with col_analisis1:
                ...
                if 'Wilayah (Kab/Kota)' in df_hasil.columns:
                    ...
                    # Grafik Wilayah
                    fig_wil = px.bar(df_wilayah.head(10), x='Jumlah', y='Wilayah', orientation='h', color='Jumlah', color_continuous_scale='Blues', text='Jumlah')
            ...
            with col_analisis2:
                ...
                if 'Nama Petugas' in df_hasil.columns:
                    df_petugas = df_hasil['Nama Petugas'].fillna('Tidak Diketahui / Kode Tidak Valid').value_counts().reset_index()
                    df_petugas.columns = ['Nama Petugas', 'Jumlah']

                    batas_tampil = st.selectbox("Pilih Tampilan Petugas:", ["Top 5", "Top 10", "Top 20", "Semua"], index=1, key="filter_petugas")
                    df_ptg_tampil = df_petugas if batas_tampil == "Semua" else df_petugas.head(int(batas_tampil.split(" ")[1]))

                    # Grafik Petugas
                    fig_petugas = px.bar(df_ptg_tampil, x='Jumlah', y='Nama Petugas', orientation='h', color='Jumlah', color_continuous_scale='Oranges', text='Jumlah')
        ...
        # --- TAB 4: MANAJEMEN WHITELIST (ADMIN) ---
        with tab4:
            ...
            if sheet_db is not None:
                ...
                if df_wl_tampil.empty:
                    ...
                    # LOGIKA ISOLASI DATA
                    if 'Lembaga' in df_hasil.columns:
                        lembaga_aktif = df_hasil['Lembaga'].dropna().unique().tolist()
                        # Lakukan filter hanya jika kolom Lembaga eksis di database
                    ...
                    if df_wl_tampil.empty:
                        ...
                        with st.form("form_cabut"):
                            ...
                            if submit_cabut:
                                try:
                                    # Mencari cell berdasarkan ID Klien
